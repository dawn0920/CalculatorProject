...
import calculatorthird.ArithmeticCalculator;
...
//        무한 루프문
        while (true) {
            // 첫번째 정수 입력
            System.out.print("첫번째 숫자(정수, 실수, 0)를 입력해주시오. : ");
            String firstInput = sc.next();
            Number numFirst = calculator.parseNumber(firstInput);
            ...
            Number numSecond;
            while (true) {
                System.out.print("두번째 숫자(정수, 실수, 0)를 입력해주시오. : ");
                String secondInput = sc.next();
                numSecond = calculator.parseNumber(secondInput);
                if (operator == '/' && (numSecond instanceof Integer && numSecond.intValue() == 0 ||
                        numSecond instanceof Double && numSecond.doubleValue() == 0)) { // int numSecond; 선언할 경우 값이 없기 떄문에 에러 발생
                    System.out.println("나눗셈 진행중... 숫자에 0이 들어갈 수 없습니다. 다시 입력해주세요.");
                } else { break; }
            }

            // 연산 및 출력을 위한 메소드 호출
            calculator.calculate(numFirst, numSecond, operator);
        ...
        System.out.print("결과값을 출력합니다. : " );
        calculator.reference(reference);
